...
function getDoughnuts() {
  $
    ...
    .done((data) => $.each(data, (index, doughnut) => addDoughnut(doughnut)));
}
...
function removeDoughnut() {
  ...
  $
    .ajax({ url: `https://ga-doughnuts.herokuapp.com/doughnuts/${id}`, method: 'DELETE' })
    .done(() => $(this).parent().remove()); // remove the button's parent (the <li>) from the dom
}
