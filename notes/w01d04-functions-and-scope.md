...
New to ES6 are JavaScript's arrow functions. They are a shorthand style of writing anonymous functions. We will be using them more throughout the course, but for now, let's just take a quick look at the syntax:
...
With ES6 we can now define _default_ arguments. If the function is called without an argument, the default is used instead:
...
function sayName(name) {
  alert(`Well hi there, ${name}`);
}

getName(sayName);
```
When we pass a function as an argument it is referred to as a **callback function**. We'll be looking at callbacks in more detail later in the course.
